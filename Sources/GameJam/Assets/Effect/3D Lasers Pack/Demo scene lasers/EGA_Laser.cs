    ...
    void Update()
    {
        ...
        if (Laser != null && UpdateSaver == false)
        {
            ...
            else
            {
                ...
                Laser.SetPosition(1, lookatObj.position);
                HitEffect.transform.position = lookatObj.position + Vector3.up * HitOffset;
                //Hit effect zero rotation
                HitEffect.transform.rotation = Quaternion.identity;
                foreach (var AllPs in Effects)
                {
                ...
                //Texture tiling
                Length[0] = MainTextureLength * (Vector3.Distance(transform.position, lookatObj.position));
                Length[2] = NoiseTextureLength * (Vector3.Distance(transform.position, lookatObj.position));
            }
